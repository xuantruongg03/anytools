"use client";

import { useLanguage } from "@/lib/i18n/LanguageContext";
import { getTranslation } from "@/lib/i18n";
import UrlEncoderClient from "./UrlEncoderClient";

export default function UrlEncoderContent() {
    const { locale } = useLanguage();
    const t = getTranslation(locale);
    const page = t.tools.urlEncoder.page;

    return (
        <>
            {/* Header Section */}
            <header className='mb-8'>
                <h1 className='text-4xl font-bold mb-4 text-gray-900 dark:text-gray-100'>{page.title}</h1>
                <p className='text-lg text-gray-600 dark:text-gray-400'>{page.subtitle}</p>
            </header>

            {/* Tool Section */}
            <UrlEncoderClient />

            {/* SEO Content Section */}
            <section className='mt-12 max-w-none'>
                <h2 className='text-2xl font-semibold mb-4 text-gray-900 dark:text-gray-100'>{page.whatIs}</h2>
                <p className='text-gray-600 dark:text-gray-400 mb-4'>{page.whatIsDesc}</p>

                <h3 className='text-xl font-semibold mb-3 text-gray-900 dark:text-gray-100'>{page.whyUse}</h3>
                <ul className='list-disc list-inside text-gray-600 dark:text-gray-400 space-y-2 mb-6'>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.features.free.split(":")[0]}:</strong> {page.features.free.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.features.instant.split(":")[0]}:</strong> {page.features.instant.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.features.bidirectional.split(":")[0]}:</strong> {page.features.bidirectional.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.features.copy.split(":")[0]}:</strong> {page.features.copy.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.features.secure.split(":")[0]}:</strong> {page.features.secure.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.features.accurate.split(":")[0]}:</strong> {page.features.accurate.split(":")[1]}
                    </li>
                </ul>

                <h3 className='text-xl font-semibold mb-3 text-gray-900 dark:text-gray-100'>{page.howToUse}</h3>
                <div className='mb-6'>
                    <h4 className='font-semibold text-gray-900 dark:text-gray-100 mb-2'>{page.encodeTitle}:</h4>
                    <ol className='list-decimal list-inside text-gray-600 dark:text-gray-400 space-y-2'>
                        <li>{page.steps.encode1}</li>
                        <li>{page.steps.encode2}</li>
                        <li>{page.steps.encode3}</li>
                    </ol>
                </div>
                <div className='mb-6'>
                    <h4 className='font-semibold text-gray-900 dark:text-gray-100 mb-2'>{page.decodeTitle}:</h4>
                    <ol className='list-decimal list-inside text-gray-600 dark:text-gray-400 space-y-2'>
                        <li>{page.steps.decode1}</li>
                        <li>{page.steps.decode2}</li>
                        <li>{page.steps.decode3}</li>
                    </ol>
                </div>

                <h3 className='text-xl font-semibold mb-3 text-gray-900 dark:text-gray-100'>{page.useCases}</h3>
                <ul className='list-disc list-inside text-gray-600 dark:text-gray-400 space-y-2 mb-6'>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.useCasesList.query.split(":")[0]}:</strong> {page.useCasesList.query.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.useCasesList.api.split(":")[0]}:</strong> {page.useCasesList.api.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.useCasesList.forms.split(":")[0]}:</strong> {page.useCasesList.forms.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.useCasesList.special.split(":")[0]}:</strong> {page.useCasesList.special.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.useCasesList.debug.split(":")[0]}:</strong> {page.useCasesList.debug.split(":")[1]}
                    </li>
                    <li>
                        <strong className='text-gray-900 dark:text-gray-100'>{page.useCasesList.seo.split(":")[0]}:</strong> {page.useCasesList.seo.split(":")[1]}
                    </li>
                </ul>

                <h3 className='text-xl font-semibold mb-3 text-gray-900 dark:text-gray-100'>{page.whatIsUrlEncoding}</h3>
                <p className='text-gray-600 dark:text-gray-400 mb-4'>{page.whatIsUrlEncodingDesc}</p>

                <h3 className='text-xl font-semibold mb-3 text-gray-900 dark:text-gray-100'>{page.faq}</h3>
                <div className='space-y-4 text-gray-600 dark:text-gray-400'>
                    <div>
                        <h4 className='font-semibold text-gray-900 dark:text-gray-100 mb-2'>{page.faqList.q1}</h4>
                        <p>{page.faqList.a1}</p>
                    </div>
                    <div>
                        <h4 className='font-semibold text-gray-900 dark:text-gray-100 mb-2'>{page.faqList.q2}</h4>
                        <p>{page.faqList.a2}</p>
                    </div>
                    <div>
                        <h4 className='font-semibold text-gray-900 dark:text-gray-100 mb-2'>{page.faqList.q3}</h4>
                        <p>{page.faqList.a3}</p>
                    </div>
                    <div>
                        <h4 className='font-semibold text-gray-900 dark:text-gray-100 mb-2'>{page.faqList.q4}</h4>
                        <p>{page.faqList.a4}</p>
                    </div>
                    <div>
                        <h4 className='font-semibold text-gray-900 dark:text-gray-100 mb-2'>{page.faqList.q5}</h4>
                        <p>{page.faqList.a5}</p>
                    </div>
                </div>
            </section>
        </>
    );
}
