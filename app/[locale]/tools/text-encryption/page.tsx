...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const title = isVi 
        ? "Mã Hóa & Giải Mã Văn Bản - AES Encryption Tool Miễn Phí 2026" 
        : "Text Encryption & Decryption - Free AES Encryption Tool 2026";
    const description = isVi 
        ? "Công cụ mã hóa văn bản miễn phí với AES-256, ROT13, Caesar cipher. Bảo vệ dữ liệu an toàn, xử lý 100% trên trình duyệt. Không giới hạn sử dụng." 
        : "Free online text encryption tool with AES-256, ROT13, Caesar cipher. Secure your sensitive data with strong encryption. 100% browser-based, unlimited use.";
...
export default async function TextEncryptionPage({ params }: Props) {
    ...
    return (
        <ToolPageLayout 
            title={isVi ? "Mã Hóa & Giải Mã Văn Bản" : "Text Encryption & Decryption"} 
            description={isVi 
                ? "Mã hóa và giải mã văn bản với AES-256, ROT13, và Caesar cipher. Bảo mật tuyệt đối, xử lý hoàn toàn trên trình duyệt của bạn." 
