...
export default function HomeContent({ locale }: HomeContentProps) {
    ...
    const filteredTools = allTools.filter((tool) => {
        const toolData = t.tools[tool.key as keyof typeof t.tools] as { name?: string; description?: string };
                ...
                {displayTools.length > 0 ? (
                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto'>
                        {displayTools.map((tool) => {
                            const toolData = t.tools[tool.key as keyof typeof t.tools] as { name?: string; description?: string; category?: string };
