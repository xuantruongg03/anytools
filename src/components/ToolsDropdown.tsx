...
export function ToolsDropdown() {
    ...
    return (
        <div className='relative' onMouseEnter={handleMainMouseEnter} onMouseLeave={handleMainMouseLeave}>
            {/* Trigger Button */}
            <button className='flex items-center gap-1 text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 font-medium'>
            ...
            {isOpen && (
                <div className='absolute top-full left-0 mt-2 w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl animate-fadeIn'>
                    {toolsConfig.map((category) => (
                        <div key={category.key} className='relative group/item'>
                            {/* Category Item */}
                            <div className='flex items-center justify-between px-4 py-3 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors cursor-pointer' onMouseEnter={() => setHoveredCategory(category.key)}>
                            ...
                            {hoveredCategory === category.key && (
                                <div className='absolute left-full top-0 ml-1 w-72 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl animate-fadeIn z-50' onMouseEnter={() => setHoveredCategory(category.key)} onMouseLeave={() => setHoveredCategory(null)}>
                                    <div className='py-1 max-h-[70vh] overflow-y-auto' style={{ scrollbarWidth: "thin", scrollbarColor: "rgb(156 163 175) transparent" }}>
                                        {category.tools.map((tool) => {
                                            ...
                                            return (
                                                <Link key={tool.href} href={`/${locale}${tool.href}`} className='flex items-center gap-3 px-4 py-2.5 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors group'>
